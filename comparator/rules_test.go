...
func TestSameValueAfterChangeRule_CreateViolationInThreshold(t *testing.T) {
	...
	fieldDifferences := []EventFieldDifference{
		{
			OldRecord:       "value1",
			NewRecord:       "value1",
...
func TestFieldChangeCountRule_CheckForViolation(t *testing.T) {
	...
	fieldName := "myField"
	differences := []EventFieldDifference{
	...
	differences = append(differences, EventFieldDifference{SourceEventId: 4, OldRecord: "oldValue4", NewRecord: "newValue4"})
	result = rule.CheckForViolation(fieldName, differences)
